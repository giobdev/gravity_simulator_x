...
import sys
from runbis import start_simulation, random

#Lista per memorizzare i corpi inseriti
bodies = []

#Variabile per memorizzare l'oggetto Main (simulazione)
simulation = None

#Funzione per la pagina iniziale con due bottoni
def create_gui():
    
    #Funzione per passare alla finestra di inserimento manuale dei corpi
    ...
    #Imposta una larghezza di 500px e un'altezza di 400px per la finestra principale
    ...
    #La funzione 
    random_button = tk.Button(root, text="Start random simulation", font=("Helvetica", 12), bg="midnight blue", fg="white", command=random)
...
def manual_input_window():
    ...
    def add_body_gui():
        ...
        #Sticky = "w" significa allineamento al lato sinistro della cella, cioè verso la direzione "West" 
        tk.Label(add_body_frame, text="Mass (0.1 to 15 * 10^23 kg):", fg="white", bg="black", font=("Helvetica", 12)).grid(row=1, column=0, sticky="w")

        #Width = 7 significa che il campo di input deve essere largo abbastanza da ccontenere 7 caratteri
        mass_entry = tk.Entry(add_body_frame, font=("Helvetica", 12), bg="black", fg="white",width=7)
        #Padx=(5,0) signfica che si sta applicando 5 pixel di spazio a sinistra del widget e 0 pixel di spazio a destra
    ...
    def reset_simulation():
        global simulation
        # Chiudi la finestra di input corrente
        root.quit()
        root.destroy()  # Chiude la finestra
        bodies.clear()  # Resetta la lista dei corpi
        manual_input_window()  # Riavvia la finestra di inserimento manuale

    def restart_simulation():
        global simulation
        if simulation:
            simulation.restart()  # Riavvia la simulazione
            simulation.mainLoop()  # Entra nel ciclo della simulazione
    ...
    #Pady=20 è un parametro che si usa per aggiungere spazio verticaletra un widget ed un altro
    control_frame.pack(pady=20)

    #Highlightthickness=0 rimuove lo spessore del bordo 
    canvas = tk.Canvas(control_frame, width=248, height=80, bg="black", bd=0, highlightthickness=0)
    #Padx=10  significa che il widget avrà uno spazio di 10 pixel extra sia a sinistra che a destra
    canvas.grid(row=0, column=0, padx=10)

    #Create_ovale disegna i bottoni
    ...
    #Create_text aggiunge un testo ai bottoni
    ...
    #Tag_bind collega i bottoni alle funzioni
    #<Button-1> significa che l'evento si attiva con un clic del tasto sinistro del mouse
    canvas.tag_bind(start_button, "<Button-1>", lambda event: start_simulation(bodies))
    canvas.tag_bind(reset_button, "<Button-1>", reset_simulation)
    canvas.tag_bind(restart_button, "<Button-1>", restart_simulation)
